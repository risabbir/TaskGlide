...
import { DEFAULT_COLUMNS, PRIORITIES, PRIORITY_STYLES, DEFAULT_SORT_STATE, DEFAULT_FILTER_STATE } from "@/lib/constants";
import type { Priority, SortCriteria, SortDirection } from "@/lib/types";
import { ScrollArea } from "@/components/ui/scroll-area";

export function FilterSidebar() {
  const { state, dispatch } = useKanban();
  const { filters, sort, columns, isFilterSidebarOpen } = state;
  ...
  const handleStatusChange = (columnId: string, checked: boolean) => {
    ...
    // If all are unchecked, effectively means all are selected for display logic elsewhere, or show "no selection" state.
    // For filter pills and logic, if it's empty, it means no filter.
    // If it's empty after unchecking, reset to all columns to avoid an empty board.
          ...
          <div className="p-6 space-y-6">
            ...
            <div>
              ...
              <RadioGroup
                ...
                onValueChange={(value) => handlePriorityChange(value as Priority | undefined)}
            ...
            <div>
              ...
              <RadioGroup
                ...
                onValueChange={(value) => handleDueDateChange(value as any)}
                className="mt-2 space-y-1"
              >
                {[
